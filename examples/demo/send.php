...
if($submit && isset($submit))
{
	...
	if($options->type == 'LMS' || $options->type == 'MMS')
		$options->subject = filter_input(INPUT_POST, 'subject');
	$reserve_time = filter_input(INPUT_POST, 'reserve_time');
	if(preg_match('/\d{4}\d{2}\d{2}\d{2}\d{2}\d{2}/',$reserve_time)){
		$options->datetime = $reserve_time;
	}
	$result = new stdClass();
...
<form class="form-horizontal" method="post">
	...
	<div class="control-group">
		<label class="control-label" for="reserve_msg">Reservation</label>
		<div class="controls">
			<input type="text" id="reserve_time" name="reserve_time" 
				placeholder="ex)  20131216090510" />
		</div>
	</div>
